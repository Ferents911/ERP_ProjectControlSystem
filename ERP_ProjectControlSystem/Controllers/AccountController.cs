    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                IdentityResult account = await _userManager.CreateAsync(user, model.Password);

                IdentityResult role = await _userManager.AddToRoleAsync(user, model.IsAdmin ? "Admin" : "User");

                if (account.Succeeded && role.Succeeded)
                {
